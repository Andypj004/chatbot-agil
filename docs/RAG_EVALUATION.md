...
Las preguntas se diseñaron para cubrir los subtemas centrales del corpus de 14 PDFs ya ingeridos (la Scrum Guide, Essential Scrum, el glosario asociado, y la guía condensada de Kanban en español, entre otros). Cada `anchor_contains` fue verificado manualmente contra el índice real de ChromaDB: se confirmó que la frase ancla aparece literalmente (tras normalización) en el contenido del chunk que el sistema efectivamente recupera para esa fuente, y que `source_contains` matchea el nombre de archivo real del PDF ingerido. Esto significa que el ground truth no es hipotético — fue calibrado contra el corpus real, no contra una versión idealizada del contenido.
...
**Techo estructural de Context Precision con el dataset actual**: las 16 preguntas del dataset tienen exactamente **un solo descriptor** en `relevant_chunks` (un único chunk "dorado" marcado a mano). Con `k` documentos recuperados, el máximo valor alcanzable de `context_precision` es `1/k` — aunque el retriever fuera perfecto y siempre incluyera el chunk dorado en el resultado, los `k-1` documentos restantes nunca contarán como relevantes según esta métrica (no matchean ese único descriptor, aun cuando en la práctica traten el mismo tema). Por eso, con `k=5`, `context_precision` solo puede tomar los valores `0.0` (chunk dorado ausente del top-k) o `0.2` (chunk dorado presente) en este dataset — nunca un valor intermedio ni mayor. Esto no es indicio de que el retriever traiga contenido irrelevante; es una consecuencia directa de tener un solo positivo verdadero por pregunta. Ver la sección 8 para un ejemplo numérico real.
...
Aclaración de granularidad: el recall se calcula por **descriptor**, no por documento recuperado. Si una pregunta tiene un solo descriptor en `relevant_chunks` (como ocurre en las 16 preguntas actuales del dataset) y dicho descriptor es encontrado por cualquiera de los documentos recuperados, el recall de esa pregunta es `1.0` — no importa cuántos documentos irrelevantes se recuperaron también, ni importa si el descriptor fue matcheado por el primer o el último documento de la lista. El recall solo decae cuando hay múltiples descriptores y algunos no son cubiertos por ningún documento recuperado.
...
**Cómo leer un MRR global "bajo"**: conviene descomponerlo en dos factores antes de interpretarlo como un problema de ranking. (1) La fracción de preguntas para las que el chunk dorado ni siquiera aparece en el top-k (esas preguntas aportan `0.0` al promedio y son indistinguibles, en el agregado, de un acierto en el último puesto). (2) El MRR promedio condicionado *solo* a las preguntas donde sí se encontró el chunk dorado, que indica en qué posición típica aparece cuando la recuperación funciona. Un MRR global moderado puede ocultar una recuperación con buen ranking (MRR condicional alto) arrastrada hacia abajo simplemente por una tasa de aciertos incompleta — son dos causas distintas con remedios distintos (subir `k`/mejorar el embedding vs. ajustar el orden de ranking). Ver la sección 8 para un ejemplo numérico real con los resultados de la corrida completa.
...
- **Context Precision tiene un techo bajo por diseño, y un Context Recall/MRR moderados pueden venir de una sola causa concentrada**: con un único descriptor por pregunta en el dataset actual (sección 4), `context_precision` solo puede valer `1/k` o `0.0` — nunca más. Ejemplo real de una corrida completa con `k=5` (`TOP_K_RESULTS=5` en `.env`) sobre las 16 preguntas:
  - `context_precision` tomó únicamente los valores `0.0` y `0.2` en las 16 preguntas (jamás un valor intermedio ni mayor a `0.2 = 1/5`), confirmando el techo estructural.
  - El promedio global de `context_precision` (`0.14`) y de `context_recall` (`0.69`) resultaron bajos casi en su totalidad porque **5 de las 16 preguntas (31 %)** no encontraron el chunk dorado dentro del top-5 (`context_precision = context_recall = mrr = 0.0` en esas 5), mientras que las **11 restantes alcanzaron el techo de `0.2`/`1.0`** en precision/recall respectivamente.
  - El MRR global (`0.49`) se descompone como: `11/16` preguntas con acierto, con un MRR promedio condicional de `0.72` entre esas 11 (equivalente a encontrar el chunk dorado típicamente en el puesto 1–2). El resto de la caída hacia `0.49` viene enteramente de las 5 preguntas sin acierto, no de que el chunk dorado aparezca consistentemente en posiciones bajas cuando sí es recuperado.
  - Inspeccionando esas 5 preguntas sin acierto (p. ej. `retrospectivas-01`/`retrospectivas-02`), el retriever sí trajo chunks reales y temáticamente correctos (de "The Scrum Guide" y el "Glosario-Essential Scrum") entre los 5 resultados — simplemente no era el chunk verbatim específico anclado a mano en el ground truth, que en al menos un caso aparecía recién en el puesto ~17 de una búsqueda con `k` ampliado. Esto es una limitación del ground truth de un solo chunk por pregunta combinada con un `k` pequeño, no evidencia de que el retriever real devuelva contenido irrelevante.
  - Implicación práctica: antes de interpretar un `context_precision`/`mrr` "bajo" como un problema real de retrieval, conviene (a) revisar si el dataset tiene más de un descriptor relevante por pregunta para esa pregunta puntual, y (b) probar con un `--k` más alto en el script de evaluación para distinguir entre "el chunk dorado nunca aparece" y "aparece pero en una posición baja".
