...
class Settings(BaseSettings):
    ...
    conversation_db_path: str = Field(
        default="./data/conversations.db", alias="CONVERSATION_DB_PATH"
    )
    conversation_context_messages: int = Field(
        default=12, alias="CONVERSATION_CONTEXT_MESSAGES"
    )
    conversation_list_limit: int = Field(default=50, alias="CONVERSATION_LIST_LIMIT")

    # Startup / lifecycle
    warmup_vector_store_on_startup: bool = Field(
        default=True, alias="WARMUP_VECTOR_STORE_ON_STARTUP"
    )
