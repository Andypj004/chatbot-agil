...
def test_retrieve_documents_combines_global_and_session():
    ...
    global_docs = [
        Document(
            page_content="global a", metadata={"scope": "global_rag", "file_hash": "g1"}
        ),
        Document(
            page_content="global b", metadata={"scope": "global_rag", "file_hash": "g2"}
        ),
        Document(
            page_content="global c", metadata={"scope": "global_rag", "file_hash": "g3"}
        ),
    ]
    session_docs = [
        Document(
    ...
    retriever = RAGRetriever(
        vector_store=vector_store, llm_provider=_DummyProvider(), top_k=5
    )
...
def test_retrieve_documents_without_session_returns_only_global():
    ...
    global_docs = [
        Document(
            page_content="global only",
            metadata={"scope": "global_rag", "file_hash": "g-only"},
        ),
    ]
    ...
    retriever = RAGRetriever(
        vector_store=vector_store, llm_provider=_DummyProvider(), top_k=3
    )
...
def test_query_overfetches_sources_when_return_sources_enabled():
    ...
    global_docs = [
        Document(
            page_content=f"global {idx}",
            metadata={"scope": "global_rag", "file_hash": f"g{idx}"},
        )
